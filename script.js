...
function displayWeather(){
  getDailyForecast().then((data) => {
    for (let i = 1; i < 6; i++){
      ...
      weekDay.innerText = days[new Date(data.list[i].dt_txt).getDay()]
      let maxTemp = dayDiv.getElementsByClassName("max-temp")[0]
      maxTemp.innerText = data.list[i].main.temp_max + units.temperature
  
      let minTemp = dayDiv.getElementsByClassName("min-temp")[0]
      minTemp.innerText = data.list[i].main.temp_min + units.temperature
  
      determineWeatherIcon(dayDiv.getElementsByClassName("path-1")[0], dayDiv.getElementsByClassName("path-2")[0])
    }
  ...
  getCurrentWeather().then((data) => {
    ...
    determineWeatherIcon(document.getElementById("p1"), document.getElementById("p2"))
  })
}

function determineWeatherIcon(svgFirstPath, svgSecondPath) {
